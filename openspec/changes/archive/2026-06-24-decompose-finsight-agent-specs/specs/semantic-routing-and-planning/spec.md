...
- 输入问题的 intent 判别、follow-up type 判别与 V1 四阶段 plan 骨架
...
### Requirement: Planner 输出稳定的 V1 四阶段骨架
系统 MUST 把受支持的 router 输出转换为一个受约束的 V1 plan，该 plan 由 `collect_event_context`、`analyze_targets`、`retrieve_evidence` 和 `synthesize_report` 四类阶段组成。
