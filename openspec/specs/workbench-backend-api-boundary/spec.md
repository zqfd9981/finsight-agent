...
定义 FinSight Agent V1 工作台到后端的统一分析 API 边界，确保 `frontend/streamlit_app` 通过稳定 request/response contract 与 `backend/apps/api` 交互，而不是直接依赖后端内部模块。
...
- **THEN** 请求体 MUST 至少支持 `query` 必填字段，并允许通过 optional `session_id` 表达会话续接，通过 optional `include_trace` 标记表达是否请求 trace 数据
