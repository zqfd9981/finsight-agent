...
结构化市场数据能力仍处于首版最小支撑阶段：

- 已足够支撑当前 `metric_lookup` 快路径
- 但还没有接入更真实、更可扩展的指标数据源
...
- `structured-market-data-support` 仍未接入真实指标源
- retrieval 评测样本仍不足，回归主要依赖单测与集成测试
- evidence follow-up 在线链路仍受 `SessionContext` 未接线限制
...
- retrieval 继续保持稳定可消费
- `structured-market-data-support` 接入更真实的数据源
- 补齐首批评测样本与回归验证入口
