"""海天味业 (603288) 重跑修复后的 TableExtractor + 入库 + 验证。

现状（修复前）：591 条记录，notes=0（注释区完全缺失）。
分布：balance_sheet=172, cash_flow_statement=116, equity_statement=113,
      income_statement=113, unknown=77。
unknown=77 说明之前 source_section 识别也有问题。
...
STRUCT_DIR = REPO / "var/data/parsed_filings/603288_海天味业__annual__2025__603288_海天味业_annual_report_2025_20250403__structured"
TABLES_JSONL = STRUCT_DIR / "tables.jsonl"
DOC_ID = "603288_海天味业__annual__2025__603288_海天味业_annual_report_2025_20250403__structured"

print("=" * 80)
print("海天味业 (603288) 重跑 TableExtractor（修复后）")
print("=" * 80)

# 1. 删除旧 SQLite 记录
conn = sqlite3.connect(str(DB_PATH))
cur = conn.cursor()
cur.execute("DELETE FROM metric_records WHERE company_code = '603288'")
...
repo.save_records_for_company("603288", records)
print(f"   入库完成")
