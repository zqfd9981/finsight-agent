"""赛力斯 (601127) 重跑修复后的 TableExtractor + 入库 + 验证。

现状：635 条记录，notes=0（注释区完全缺失），分布：
  balance_sheet=175, cash_flow_statement=127, equity_statement=214, income_statement=119
重跑修复后的 TableExtractor（分批 LLM 决策 + 漏返回兜底 keep=1）即可恢复注释区。
"""
import sys
import sqlite3
...
STRUCT_DIR = REPO / "var/data/parsed_filings/601127_赛力斯__annual__2025__601127_赛力斯_annual_report_2025_20250401__structured"
TABLES_JSONL = STRUCT_DIR / "tables.jsonl"
DOC_ID = "601127_赛力斯__annual__2025__601127_赛力斯_annual_report_2025_20250401__structured"

print("=" * 80)
print("赛力斯 (601127) 重跑 TableExtractor（修复后）")
print("=" * 80)

# 1. 删除旧 SQLite 记录
conn = sqlite3.connect(str(DB_PATH))
cur = conn.cursor()
cur.execute("DELETE FROM metric_records WHERE company_code = '601127'")
...
repo.save_records_for_company("601127", records)
print(f"   入库完成")
